...
export default function Page() {
  ...
  return <main id="top" className="min-h-screen overflow-hidden bg-background text-foreground">
    ...
    <header className="sticky top-0 z-50 border-b border-border/70 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
      <a href="#top" className="flex items-center gap-3"><img src={assets.logo} alt="VENUS logo" className="h-11 w-28 object-contain" /><span className="hidden border-l border-border pl-3 text-[10px] font-black uppercase leading-tight tracking-[.18em] text-navy sm:block">Superior Plastic<br />Enterprises</span></a>
    ...
    <section className="bg-surface"><div className="mx-auto grid max-w-7xl items-center gap-10 px-5 py-14 lg:grid-cols-[.82fr_1.18fr] lg:px-8 lg:py-24"><div>
      <p className="eyebrow flex items-center gap-3"><span className="h-px w-8 bg-accent" /> Established 2000</p><h1 className="mt-5 max-w-2xl text-balance text-5xl font-black leading-[1.02] tracking-tight text-navy sm:text-6xl lg:text-7xl">PVC products built for <span className="text-primary">everyday business.</span></h1><p className="mt-6 max-w-xl text-pretty text-lg leading-8 text-slate-600">Superior Plastic Enterprises manufactures dependable seat covers, handle protection, garment covers and packaging solutions for businesses across India.</p><div className="mt-8 flex flex-wrap gap-3"><Button href="#products">Explore Products</Button><Button href="#contact" outline>Enquire Now</Button></div><div className="mt-9 flex flex-wrap gap-x-8 gap-y-3 text-sm font-bold text-slate-600"><span><strong className="text-navy">25+</strong> years</span><span><strong className="text-navy">40</strong> professionals</span><span><strong className="text-navy">Pan-India</strong> supply</span></div>
    </div><img src={assets.hero} alt="Superior Plastic Enterprises seat covers, scooter covers and PVC bags" className="w-full rounded-[2rem] object-cover shadow-2xl shadow-navy/15" /></div></section>

    <section id="experience" className="border-y border-border bg-white"><div className="mx-auto grid max-w-7xl gap-12 px-5 py-20 lg:grid-cols-[.85fr_1.15fr] lg:items-center lg:px-8 lg:py-24"><div><p className="eyebrow">Our experience. Our capability.</p><h2 className="section-title">Built on <span>25+ years</span> of experience.</h2><p className="mt-6 max-w-xl text-lg leading-8 text-muted-foreground">Since 2000, Superior Plastic Enterprises has been manufacturing dependable PVC products and protective solutions for businesses across India.</p><div className="mt-8 flex items-center gap-3 text-sm font-bold text-navy"><span className="h-px w-10 bg-accent" />Made in Bhiwandi, Maharashtra</div></div><div><div className="grid grid-cols-2 border-l border-t border-border">{[['25+', 'Years of Experience', 'A quarter-century of dependable manufacturing.'], ['40+', 'Skilled Professionals', 'A dedicated team behind every requirement.'], ['2000', 'Established', 'Building trust since the new millennium.'], ['Pan India', 'Supply Network', 'Serving businesses across India.']].map(([value, label, text]) => <div key={label} className="border-b border-r border-border p-5 sm:p-7"><p className="text-3xl font-black tracking-tight text-primary sm:text-4xl">{value}</p><p className="mt-2 font-bold text-navy">{label}</p><p className="mt-2 text-sm leading-6 text-muted-foreground">{text}</p></div>)}</div></div></div></section>
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-[minmax(0,1fr)_300px] items-center gap-8">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-6 max-w-[620px] space-y-4 text-[15px] leading-7 text-slate-600">
            <p>
              Established in 2000, Superior Plastic Enterprises has more than 25
